...
exports.execute = async (message) => {
  ...
  await guildChannel.findOneAndUpdate({ guildID: message.guild.id, channelID: message.channel.name }, { $inc: { channelData: 1 } }, { upsert: true });
  await userChannel.findOneAndUpdate({ guildID: message.guild.id,  userID: message.author.id, channelID: message.channel.name }, { $inc: { channelData: 1 } }, { upsert: true });
};
