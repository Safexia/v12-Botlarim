...
exports.execute = async (client, message, args) => {
  ...
  if(message.author.id !== "728161454288535604") return message.react(client.emojis.cache.find(x => x.name === "ravgar_carpi"));
    ...
    const acik = ` ${client.emojis.cache.find(x => x.name === "ravgar_yesil")}`
    const kapali = ` ${client.emojis.cache.find(x => x.name === "ravgar_kirmizi")}`
    ...
    **Voice Mute**
    ...
    ${await db.has(`vmutebilgi_ravgar`) ? `${acik} - **VoiceMute Bilgi** (<#${await db.get(`vmutebilgi_ravgar`)}>)` : `${kapali} - \`.kanalsetup vmutebilgi #kanal\``}
    ...
    **Chat Mute**
    ...
    ${await db.has(`cmutebilgi_ravgar`) ? `${acik} - **ChatMute Bilgi** (<#${await db.get(`cmutebilgi_ravgar`)}>)` : `${kapali} - \`.kanalsetup cmutebilgi #kanal\``}
    **Jail'd**
    ...
    **Reklam**
    ...
    ${await db.has(`reklambilgi_ravgar`) ? `${acik} - **Reklam Bilgi** (<#${await db.get(`reklambilgi_ravgar`)}>)` : `${kapali} - \`.kanalsetup reklambilgi #kanal\``}
